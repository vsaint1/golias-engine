...
bool RenderingDeviceSDL_GPU::initialize(SDL_Window* sdl_window) {
    ...
    _device = SDL_CreateGPUDevice(SDL_GPU_SHADERFORMAT_SPIRV | SDL_GPU_SHADERFORMAT_MSL | SDL_GPU_SHADERFORMAT_DXIL, true, device);
...
RID RenderingDeviceSDL_GPU::texture_create(const TextureFormat& format, void* data) {
    ...
    create_info.num_levels           = format.mipmaps;
    create_info.sample_count         = (SDL_GPUSampleCount) format.samples;
...
void RenderingDeviceSDL_GPU::sampler_destroy(RID sampler) {
    ...
    if (it != _samplers.end()) {
        if (it->second) SDL_ReleaseGPUSampler(_device, it->second);
        _samplers.erase(it);
    }
}
...
RID RenderingDeviceSDL_GPU::pipeline_create(const PipelineState& state) {
 auto shader_it = _shaders.find(state.shader);
    if (shader_it == _shaders.end()) return INVALID_RID;
    ...
    rasterizer.front_face = state.rasterization.front_face_ccw ?
    ...
    // depth_stencil.reference = (Uint8)state.depth_stencil.stencil_reference;
    ...
    for (size_t i = 0; i < state.blend_states.size(); ++i) {
        ...
        if (blend.write_r) bs.color_write_mask |= SDL_GPU_COLORCOMPONENT_R;
        if (blend.write_g) bs.color_write_mask |= SDL_GPU_COLORCOMPONENT_G;
        if (blend.write_b) bs.color_write_mask |= SDL_GPU_COLORCOMPONENT_B;
        if (blend.write_a) bs.color_write_mask |= SDL_GPU_COLORCOMPONENT_A;
...
void RenderingDeviceSDL_GPU::pipeline_destroy(RID pipeline) {
    ...
    if (it != _pipelines.end()) {
        if (it->second) SDL_ReleaseGPUGraphicsPipeline(_device, it->second);
        _pipelines.erase(it);
    }
}
...
void RenderingDeviceSDL_GPU::render_pass_begin(RID framebuffer, const Viewport& viewport, const Scissor& scissor) {
    ...
    if (fb_it != _framebuffers.end()) {
        for (const auto& attachment : fb_it->second) {
            ...
            if (tex_it == _textures.end()) continue;
            ...
            if (tex_it->second.format.depth_stencil) {
                ...
                // depth_target.clear_value.depth = attachment.clear_value.depth;
                // depth_target.clear_value.stencil = (Uint8)attachment.clear_value.stencil;
        ...
        if (SDL_AcquireGPUSwapchainTexture(_cmd_buffer, _window, &swapchain, &width,&height)) {
            ...
            color_targets[0].clear_color.r = 0.0f;
            color_targets[0].clear_color.g = 0.0f;
            color_targets[0].clear_color.b = 0.0f;
            color_targets[0].clear_color.a = 1.0f;
            num_color_targets = 1;
        }
    }

    _render_pass = SDL_BeginGPURenderPass(
        _cmd_buffer,
        color_targets,
        num_color_targets,
        has_depth ? &depth_target : nullptr
    );
