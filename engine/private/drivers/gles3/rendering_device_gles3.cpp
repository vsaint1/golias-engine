...
GLuint RenderingDeviceGLES3::compile_shader(GLenum type, const std::string& source) {
...
RID RenderingDeviceGLES3::shader_create_from_source(const std::string& vertex_src, const std::string& fragment_src) {
    ...
    shaders[rid] = ShaderData{program};
    return rid;
}
...
RID RenderingDeviceGLES3::buffer_create(size_t size, uint32_t usage_flags, const void* data) {
    ...
    RID rid      = allocate_rid();
    buffers[rid] = BufferData{buffer, size, usage_flags, target};
    return rid;
}
...
RID RenderingDeviceGLES3::texture_create(const TextureFormat& format, void* data) {
    ...
    RID rid       = allocate_rid();
    textures[rid] = TextureData{texture, format};
...
RID RenderingDeviceGLES3::pipeline_create(const PipelineState& state) {
    ...
    RID rid        = allocate_rid();
    pipelines[rid] = PipelineData{state, vao};
    return rid;
}
...
void RenderingDeviceGLES3::apply_blend_state(const std::vector<BlendState>& states) {
...
void RenderingDeviceGLES3::bind_vertex_buffers(const std::vector<RID>& buffer_rids, const std::vector<size_t>& offsets) {
...
void RenderingDeviceGLES3::bind_uniform_buffer(uint32_t binding, RID buffer, size_t offset, size_t size) {
    ...
    if (it != buffers.end()) {
        ...
        glBindBufferRange(GL_UNIFORM_BUFFER, binding, it->second.buffer, offset, size);
    }
}
...
RID RenderingDeviceGLES3::sampler_create(const SamplerState& state) {
    ...
    RID rid       = allocate_rid();
    samplers[rid] = SamplerData{sampler, state};
    return rid;
}
...
RID RenderingDeviceGLES3::framebuffer_create(const std::vector<RenderPassAttachment>& attachments) {
    GLuint fbo;
    ...
    for (const auto& attachment : attachments) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, attachment_point, GL_TEXTURE_2D, tex.texture, attachment.mip_level);
    }
    ...
    RID rid           = allocate_rid();
    framebuffers[rid] = FramebufferData{fbo, attachments, width, height};
    return rid;
}
...
void RenderingDeviceGLES3::push_constant(const std::string& name, const void* data, size_t size) {
    if (current_pipeline == INVALID_RID) {
        return;
    }
