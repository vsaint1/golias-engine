...
RenderingDeviceGLES3::~RenderingDeviceGLES3() { shutdown(); }
...
RID RenderingDeviceGLES3::buffer_create(size_t size, uint32_t usage_flags,
                                        const void *data) {
  GLuint buffer;
  glGenBuffers(1, &buffer);

  GLenum target = (usage_flags & (uint32_t)BufferUsage::UNIFORM)
                      ? GL_UNIFORM_BUFFER
                      : GL_ARRAY_BUFFER;
  ...
  RID rid = allocate_rid();
  buffers[rid] = Buffer{buffer, size, usage_flags, target};
  return rid;
}

void RenderingDeviceGLES3::buffer_update(RID buffer, size_t offset, size_t size,
                                         const void *data) {
...
RID RenderingDeviceGLES3::texture_create(const TextureFormat &format,
                                         void *data) {
  GLuint texture;
  ...
  if (format.type == TextureType::TYPE_2D) {
    if (data == nullptr) {
      spdlog::error("TextureCreate called with null data!");
      glTexImage2D(GL_TEXTURE_2D, 0, internal_format, format.width,
                   format.height, 0, pixel_format, pixel_type, nullptr);
    } else {
      glTexImage2D(GL_TEXTURE_2D, 0, internal_format, format.width,
                   format.height, 0, pixel_format, pixel_type, data);
    }
  }
  ...
  RID rid = allocate_rid();
  textures[rid] = Texture{texture, format};
...
void RenderingDeviceGLES3::get_texture_size(RID texture, uint32_t &width,
                                            uint32_t &height) {
  auto it = textures.find(texture);
  if (it != textures.end()) {
...
void RenderingDeviceGLES3::apply_rasterization_state(
    const RasterizationState &state) {
  if (state.cull_mode == CullMode::NONE) {
    glDisable(GL_CULL_FACE);
  } else {
    glEnable(GL_CULL_FACE);
    GLenum mode = (state.cull_mode == CullMode::FRONT)  ? GL_FRONT
                  : (state.cull_mode == CullMode::BACK) ? GL_BACK
                                                        : GL_FRONT_AND_BACK;
    glCullFace(mode);
  }
...
void RenderingDeviceGLES3::apply_depth_stencil_state(
...
void RenderingDeviceGLES3::apply_blend_state(
  ...
  if (states.empty() || !states[0].enable) {
    ...
    glBlendFuncSeparate(to_gl_blend_factor(blend.src_color),
                        to_gl_blend_factor(blend.dst_color),
                        to_gl_blend_factor(blend.src_alpha),
                        to_gl_blend_factor(blend.dst_alpha));
    glBlendEquationSeparate(to_gl_blend_op(blend.color_op),
                            to_gl_blend_op(blend.alpha_op));
  }
}
...
void RenderingDeviceGLES3::bind_vertex_buffers(
  ...
  for (size_t i = 0; i < buffer_rids.size(); ++i) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, buf_it->second.buffer);
    ...
    for (const auto &attr : vertex_format.attributes) {
      ...
      glVertexAttribPointer(attr.location, size, type, normalized,
                            vertex_format.stride, (void *)offset);
    }
  }
}

void RenderingDeviceGLES3::bind_index_buffer(RID buffer, IndexType type,
                                             size_t offset) {
  auto it = buffers.find(buffer);
  if (it != buffers.end()) {
...
void RenderingDeviceGLES3::draw(uint32_t vertex_count, uint32_t instance_count,
...
void RenderingDeviceGLES3::draw_indexed(uint32_t index_count,
  ...
  GLenum mode = to_gl_topology(it->second.state.topology);
  GLenum type = (current_index_type == IndexType::UINT16) ? GL_UNSIGNED_SHORT
                                                          : GL_UNSIGNED_INT;
...
void RenderingDeviceGLES3::bind_uniform_buffer(uint32_t binding, RID buffer,
  ...
  if (it != buffers.end()) {
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, binding, it->second.buffer, offset,
                      size);
  }
}

void RenderingDeviceGLES3::bind_texture(uint32_t binding, RID texture,
                                        RID sampler) {
  ...
  if (tex_it != textures.end()) {
    ...
    if (sampler != INVALID_RID) {
      ...
      if (samp_it != samplers.end()) {
        glBindSampler(binding, samp_it->second.sampler);
      }
    }
  }
}
...
RID RenderingDeviceGLES3::sampler_create(const SamplerState &state) {
  ...
  glSamplerParameteri(sampler, GL_TEXTURE_MIN_FILTER,
                      to_gl_filter(state.min_filter));
  glSamplerParameteri(sampler, GL_TEXTURE_MAG_FILTER,
                      to_gl_filter(state.mag_filter));
  ...
  if (state.compare_enabled) {
    ...
    glSamplerParameteri(sampler, GL_TEXTURE_COMPARE_FUNC,
                        to_gl_compare(state.compare_op));
  }
...
RID RenderingDeviceGLES3::framebuffer_create(
  ...
  for (const auto &attachment : attachments) {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, attachment_point, GL_TEXTURE_2D,
                           tex.texture, attachment.mip_level);
  }
...
void RenderingDeviceGLES3::render_pass_begin(RID framebuffer,
      ...
      if (attachment.clear) {
        ...
        if (tex_it != textures.end()) {
          if (tex_it->second.format.depth_stencil) {
            ...
            glClearColor(
                attachment.clear_value.color.r, attachment.clear_value.color.g,
...
void RenderingDeviceGLES3::texture_update(RID texture, uint32_t mip_level,
                                          uint32_t layer, const void *data,
                                          size_t size) {
  auto it = textures.find(texture);
  if (it == textures.end()) {
    return;
  ...
  if (tex.format.type == TextureType::TYPE_2D) {
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexSubImage2D(
        GL_TEXTURE_2D, mip_level, 0, 0, tex.format.width >> mip_level,
        tex.format.height >> mip_level, pixel_format, pixel_type, data);
  }
...
void RenderingDeviceGLES3::texture_generate_mipmaps(RID texture) {
  ...
  glBindTexture(target, it->second.texture);
