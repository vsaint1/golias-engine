...
namespace golias {
    ...
    OpenglTextureCubemap::OpenglTextureCubemap(const std::string& filePath) {
        ...
        if (aspectRatio >= 1.9f && aspectRatio <= 2.1f) {
            ...
            spdlog::info("OpenglTextureCubemap: Detected equirectangular format: {}x{}, face size: {}", 
                         imageWidth, imageHeight, faceSize);
        } else if (imageWidth * 4 == imageHeight * 3) {
            faceSize = imageWidth / 3;
            isVerticalCross = true;
            spdlog::info("OpenglTextureCubemap: Detected vertical cross layout: {}x{}, face size: {}", 
                         imageWidth, imageHeight, faceSize);
        } else if (imageHeight * 4 == imageWidth * 3) {
            faceSize = imageHeight / 3;
            isVerticalCross = false;
            spdlog::info("OpenglTextureCubemap: Detected horizontal cross layout: {}x{}, face size: {}", 
                         imageWidth, imageHeight, faceSize);
        } else {
        ...
        if (faceSize < 256) {
            spdlog::warn("OpenglTextureCubemap: Face size is small ({}x{}), quality may be reduced.", 
                         faceSize, faceSize);
        }
        ...
        if (isEquirectangular) {
            ProcessEquirectangular(data, imageWidth, imageHeight, faceSize, channels, isHDR, 
                                   internalFormat, formatEnum, dataType);
        } else {
            ProcessCrossLayout(data, imageWidth, imageHeight, faceSize, channels, isHDR, 
                               isVerticalCross, internalFormat, formatEnum, dataType);
        }
        ...
        spdlog::info("OpenglTextureCubemap: Successfully created cubemap from {}: {}x{} -> {}x{} faces",
                     ...
                     imageWidth, imageHeight, faceSize, faceSize);
    }
            ...
            if (i == 0) {
                ...
                if (w != faceWidth || h != faceHeight || nrChannels != channels) {
                    spdlog::error("OpenglTextureCubemap: Face {} dimensions {}x{} (channels: {}) don't match first "
                                  "face {}x{} (channels: {})", i, w, h, nrChannels, faceWidth, faceHeight, channels);
                    stbi_image_free(data);
                    loadSuccess = false;
                    break;
                }
        ...
        auto generate_face_gradient = [&](int face) {
            for (int y = 0; y < size; y++) {
                for (int x = 0; x < size; x++) {
                    ...
                    glm::vec3 color = horizon * (1.0f - t) + zenith * t + 
                                     glm::vec3(scatter * 50.0f, scatter * 50.0f, scatter * 30.0f);
    ...
    void OpenglTextureCubemap::ProcessEquirectangular(void* srcData,
        ...
        for (int face = 0; face < 6; face++) {
            if (isHDR) {
                SampleAndCopyPixels(static_cast<float*>(srcData), static_cast<float*>(faceData),
                                   srcWidth, srcHeight, faceSize, channels, face);
            } else {
                SampleAndCopyPixels(static_cast<unsigned char*>(srcData), static_cast<unsigned char*>(faceData),
                                   srcWidth, srcHeight, faceSize, channels, face);
            }
    ...
    void OpenglTextureCubemap::ProcessCrossLayout(void* srcData,
        ...
        for (int face = 0; face < 6; face++) {
            if (isHDR) {
                CopyFaceData(static_cast<float*>(srcData), static_cast<float*>(faceData),
                           srcWidth, faceSize, channels, 
                           facePositions[face].x, facePositions[face].y,
                           facePositions[face].flipVertical, facePositions[face].flipHorizontal);
            } else {
                CopyFaceData(static_cast<unsigned char*>(srcData), static_cast<unsigned char*>(faceData),
                           srcWidth, faceSize, channels,
                           facePositions[face].x, facePositions[face].y,
                           facePositions[face].flipVertical, facePositions[face].flipHorizontal);
            }
