...
// -------------------- SERVER CODE --------------------
bool Engine::initialize_server(const char* host, int port) {

    LOG_INFO("[SERVER] - Engine::create_server() - Port: %d", port);
...
void Engine::shutdown_server() {

    LOG_INFO("[SERVER] - Engine::shutdown_server()");
#if !defined(SDL_PLATFORM_EMSCRIPTEN)
    curl_global_cleanup();
#endif
...
void Engine::on_message(uint8_t type, std::function<void(ENetPeer*, const Packet&)> handler) {
