...
LineEdit::LineEdit(glm::vec2 pos, glm::vec2 size, const std::string& font_alias)
    : _font_alias(font_alias), _text(""), _cursor_pos(0), _show_cursor(true), _cursor_blink_timer(0.0), _is_focused(false),
      _is_hovered(false), _composition_text(""), _composition_cursor(0), _composition_selection_len(0), _pointer_cursor(nullptr),
      _default_cursor(nullptr) {
...
void LineEdit::draw(Renderer* renderer) {
    ...
    const std::string display_text = _text;

    std::string wrapped_text;
    int line_count = 0;
    for (size_t i = 0; i < display_text.size() && line_count < max_lines; i += max_chars_per_line) {
        size_t len = SDL_min((size_t) max_chars_per_line, display_text.size() - i);
        wrapped_text += display_text.substr(i, len);
        line_count++;
        if (i + len < display_text.size() && line_count < max_lines) {
            wrapped_text += "\n";
        }
    }

    renderer->draw_text(wrapped_text, _panel_rect.x + padding, _panel_rect.y + padding, 0, 1.0f, _style.text_color, _font_alias, _z_index);
    ...
    if (_show_cursor && _is_focused) {
        ...
        float cursor_x        = _panel_rect.x + padding + cursor_col * (char_width - 2);
        float cursor_y        = _panel_rect.y + padding + cursor_line * line_height;

        if (cursor_line < max_lines) {
            renderer->draw_rect({cursor_x, cursor_y, 2, line_height}, 0.0f, _style.text_color, true, 9999);
        }
    }
}
