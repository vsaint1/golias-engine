...
void Material::bind(Shader* shader) const {
    ...
    if ((has_features & HAS_ALBEDO_MAP) && albedo_map) {
    ...
    if ((has_features & HAS_SPECULAR_MAP) && specular_map) {
    ...
    if ((has_features & HAS_METALLIC_MAP) && metallic_map) {
    ...
    if ((has_features & HAS_ROUGHNESS_MAP) && roughness_map) {
    ...
    if ((has_features & HAS_NORMAL_MAP) && normal_map) {
    ...
    if ((has_features & HAS_AO_MAP) && ao_map) {
    ...
    if ((has_features & HAS_EMISSIVE_MAP) && emissive_map) {
...
glm::mat4 DirectionalLight3D::get_light_space_matrix(glm::mat4 camera_view, glm::mat4 camera_proj) const {
    ...
    for (int x = 0; x < 2; ++x) {
        for (int y = 0; y < 2; ++y) {
            for (int z = 0; z < 2; ++z) {
                glm::vec4 corner = invCam * glm::vec4(2.0f * x - 1.0f, 2.0f * y - 1.0f, 2.0f * z - 1.0f, 1.0f);
                corner /= corner.w;
    ...
    glm::mat4 orthoProj = glm::ortho(min_bounds.x, max_bounds.x, min_bounds.y, max_bounds.y, -max_bounds.z, -min_bounds.z);
