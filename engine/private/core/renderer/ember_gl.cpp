...
Renderer* CreateRenderer(SDL_Window* window, SDL_GLContext  gl_context,int view_width, int view_height){
    ...
    _renderer->gl_context = gl_context;
...
void DrawTexture(Texture2D tex, Rectangle rect, Color color) {
    ...
    glUniformMatrix4fv(glGetUniformLocation(renderer->shaderProgram, "u_Projection"), 1, GL_FALSE, glm::value_ptr(projection));
...
void DrawTextureEx(Texture2D texture, Rectangle source, Rectangle dest, glm::vec2 origin, float rotation, Color color) {
    ...
    float vertices[] = {
        // pos         // tex coords
        0.0f, 0.0f, 0.0f, texLeft,  texTop,
        1.0f, 0.0f, 0.0f, texRight, texTop,
        1.0f, 1.0f, 0.0f, texRight, texBottom,
        0.0f, 1.0f, 0.0f, texLeft,  texBottom,
    };
