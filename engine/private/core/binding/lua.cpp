...
void materials_binding(lua_State* L) {
    sol::state_view lua(L);

    // Texture binding
    lua.new_usertype<Texture>("Texture",
                              "id", sol::readonly_property([](Texture& t) {
                                  return t.id;
                              }),

                              "width", sol::readonly_property([](Texture& t) {
                                  return t.width;
                              }),

                              "height", sol::readonly_property([](Texture& t) {
                                  return t.height;
                              }),

                              "pitch", sol::readonly_property([](Texture& t) {
                                  return t.pitch;
                              }),

                              "path", sol::readonly_property([](Texture& t) {
                                  return std::string(t.path);
                              }),
                              ...
                              "target", sol::property(
                                  ...
                                  [](Texture& t, ETextureTarget target) {
                                      t.target = target;
                                  }
                                  ),
                              ...
                              "bind", sol::overload(
                                  ...
                                  [](Texture& t, Uint32 slot) {
                                      t.bind(slot);
                                  }
                                  ),

                              "is_valid", &Texture::is_valid
        );

    // Metallic binding
    lua.new_usertype<Metallic>("Metallic",
                               "texture", sol::property(
                                   [](Metallic& m) -> Texture* {
                                       return m.texture ? m.texture.get() : nullptr;
                                   }
                                   ),
    ...
    // AmbientOcclusion binding
    lua.new_usertype<AmbientOcclusion>("AmbientOcclusion",
                                       "texture", sol::property(
                                           [](AmbientOcclusion& ao) -> Texture* {
                                               return ao.texture ? ao.texture.get() : nullptr;
                                           }
                                           ),
    ...
    // Emission binding
    lua.new_usertype<Emission>("Emission",
                               "texture", sol::property(
                                   [](Emission& e) -> Texture* {
                                       return e.texture ? e.texture.get() : nullptr;
                                   }
                                   ),
                               ...
                               "emissive", sol::property(
                                   ...
                                   [](Emission& e, const glm::vec3& v) {
                                       e.emissive = v;
                                   }
                                   ),

                               "strength", sol::property(
                                   [](Emission& e) {
                                       return e.strength;
                                   },
                                   [](Emission& e, float v) {
    ...
    lua.new_usertype<Material>("Material",
     // Use methods instead of properties for pointer safety
     "set_albedo", [](Material& m, float r, float g, float b) {
         m.albedo.x = r;
         m.albedo.y = g;
         m.albedo.z = b;
     },

     "set_roughness", [](Material& m, float v) { m.roughness = v; },
     "get_roughness", [](Material& m) { return m.roughness; },

     "set_dissolve", [](Material& m, float v) { m.dissolve = v; },
     "get_dissolve", [](Material& m) { return m.dissolve; }
 );
...
void generate_bindings(lua_State* L) {
    ...
    lua.new_usertype<MeshInstance3D>("MeshInstance3D",
                                     ...
                                     "path", sol::property(
                                         ...
                                         [](MeshInstance3D& m, const std::string& p) {
                                             m.path = p;
                                         }
                                         ),

                                     // loaded - read-only
                                     "loaded", sol::readonly_property(
                                         [](MeshInstance3D& m) {
                                             return m.is_loaded;
                                         }
                                         ),

                                     // is_animated - read-only
                                     "is_animated", sol::readonly_property(
                                         [](MeshInstance3D& m) {
                                             return m.scene && m.scene->mAnimations && m.scene->mNumAnimations > 0;
                                         }
                                         ),

                                     // get_bones - returns vector of Bone from first non-null mesh
                                     "get_bones", sol::property(
                                         [](MeshInstance3D& m) -> std::vector<Bone> {
                                     ...
                                     "get_mesh_count", [](MeshInstance3D& m) -> int {
                                         return m.meshes.size();
                                     },

                                     "get_material", [](MeshInstance3D& m, int index) -> Material* {
                                         if (index < 0 || index >= m.meshes.size())
                                             return nullptr;
                                         if (!m.meshes[index])
                                             return nullptr;
                                         return m.meshes[index]->material.get();
                                     },

                                     "set_material", [](MeshInstance3D& m, int index, Material* mat) {
                                         if (index < 0 || index >= m.meshes.size())
                                             return;
                                         if (!m.meshes[index])
                                             return;
                                         m.meshes[index]->material.reset(mat);
                                     }
    ...
    lua
        .
        new_usertype<EngineConfig>(
            "EngineConfig"
            ,
            "get_viewport"
            ,
            &
            EngineConfig::get_viewport
            ,
            "get_renderer_device"
            ,
            &
            EngineConfig::get_renderer_device
            ,
            "get_window"
            ,
            &
            EngineConfig::get_window
            );

    // Viewport - use sol::property for simple types too for consistency
    lua
        .
        new_usertype<Viewport>(
            "Viewport"
            ,
            "width"
            ,
...
void push_entity_table_to_lua(lua_State* L, flecs::entity e) {
    ...
    meta[sol::meta_function::index] = [](sol::table self, const std::string& key) -> sol::object {
        ...
        if (key == "mesh" && entity.has<MeshInstance3D>()) {
            return sol::make_object(lua, entity.get_mut<MeshInstance3D>());
        }
    ...
    entity["find_child"] = [](sol::table self, const char* name, sol::optional<bool> recursive,
                              sol::optional<const char*> component) -> sol::object {
        sol::state_view lua = self.lua_state();
        flecs::entity_t id  = self["id"];
        auto entity         = GEngine->get_world().entity(id);
        flecs::entity result;
    ...
    entity["has"] = [](sol::table self, const char* component) -> bool {
        ...
        if (strcmp(component, "Transform3D") == 0)
            return entity.has<Transform3D>();
        if (strcmp(component, "Transform2D") == 0)
            return entity.has<Transform2D>();
        if (strcmp(component, "Camera3D") == 0)
            return entity.has<Camera3D>();
        if (strcmp(component, "MeshInstance3D") == 0)
            return entity.has<MeshInstance3D>();
        if (strcmp(component, "Shape2D") == 0)
            return entity.has<Shape2D>();
        if (strcmp(component, "Script") == 0)
            return entity.has<Script>();
