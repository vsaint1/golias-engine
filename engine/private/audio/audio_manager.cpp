...
namespace golias {
    ...
    AudioManager::AudioManager() {
        maDevice  = new ma_device();
        maContext = new ma_context();
    ...
    AudioManager::~AudioManager() {
        if (isReady) {
            ...
            ma_mutex* mutex = static_cast<ma_mutex*>(maMutex);
    ...
    bool AudioManager::Initialize() {
        ...
        ma_result result = ma_context_init(nullptr, 0, &ctxConfig, maContext);
        ...
        result = ma_device_init(maContext, &config, maDevice);
        if (result != MA_SUCCESS) {
        ...
        spdlog::info("    > Format:        {} -> {}",
                     ma_get_format_name(maDevice->playback.format),
                     ma_get_format_name(maDevice->playback.internalFormat));
    ...
    void AudioManager::SetMasterVolume(float volume) {
        masterVolume = volume;
        if (isReady) {
            ma_device_set_master_volume(maDevice, masterVolume);
        }
    }
    ...
    static ma_uint32 ReadAudioBufferFramesInInternalFormat(rAudioBuffer* audioBuffer, void* framesOut, ma_uint32 frameCount) {
        ...
        while (framesRemainingInOutputBuffer > 0 && (audioBuffer->frameCursorPos < audioBuffer->sizeInFrames)) {
            ...
            if (framesToRead > framesRemainingInSubBuffer) {
                framesToRead = framesRemainingInSubBuffer;
            }
    ...
    static ma_uint32 ReadAudioBufferFramesInMixingFormat(rAudioBuffer* audioBuffer, float* framesOut, ma_uint32 frameCount) {
        ...
        while (totalOutputFramesProcessed < frameCount) {
            ...
            if (inputFramesToProcess > audioBuffer->sizeInFrames) {
                inputFramesToProcess = audioBuffer->sizeInFrames;
            }
            ...
            ma_data_converter_process_pcm_frames(&audioBuffer->converter,
                                                 inputBuffer,
                                                 &inputFramesProcessedThisIteration,
                                                 runningFramesOut,
                                                 &outputFramesProcessedThisIteration);

            totalOutputFramesProcessed += (ma_uint32) outputFramesProcessedThisIteration;

            if (inputFramesProcessedThisIteration < inputFramesToProcess) {
                break;
            }
