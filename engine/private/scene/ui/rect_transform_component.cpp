        ...
        if (mStretchToParent) {
            ...
            if (parent) {
                if (auto parentRect = parent->GetComponent<RectTransformComponent>()) {
                    ...
                    bool parentIsRoot = !grandParent || !grandParent->GetComponent<RectTransformComponent>();

                    if (parentIsRoot) {
                        float uiScale = Engine::GetInstance().GetSceneRenderer().GetUIScale();
                        if (uiScale > 0.0f) {
                            const auto& viewport = Engine::GetInstance().GetSceneRenderer().GetRenderingDevice()->GetViewport();
                            return glm::vec2(static_cast<float>(viewport.width), static_cast<float>(viewport.height)) / uiScale;
                        }
                    }
