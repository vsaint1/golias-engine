...
private:
    ...
    static Material load_material(const aiScene* scene, aiMesh* mesh, const std::string& directory, Renderer& renderer);

    static void load_colors(aiMaterial* aiMat, Material& mat);

    static void load_textures(aiMaterial* aiMat, const aiScene* scene, const std::string& dir, Renderer& renderer, Material& mat);
