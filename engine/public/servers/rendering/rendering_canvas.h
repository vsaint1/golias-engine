...
namespace golias {
    ...
    public:
        ...
        void draw_circle_outlined(float x, float y, float radius, const Color& color = Color::WHITE, float thickness = 1.0f,
                                  int segments = 32);
        ...
        void draw_texture_rect(RID texture, const Rect& dest, const Rect& source, const Color& tint = Color::WHITE,
                               float rotation = 0.0f);
        ...
        void draw_custom(RID shader, float x, float y, float width, float height, RID texture = INVALID_RID,
                         const Color& color = Color::WHITE);

        void draw_texture_ex(float x, float y, float width, float height, RID texture = INVALID_RID,
                             const Rect& source = {0, 0, 0, 0}, const Color& color = Color::WHITE, float rotation = 0.0f,
                             bool flip_h = false, bool flip_v = false, RID shader = INVALID_RID);
...
inline void RenderingCanvas::draw_text(TTF_Font* font, float x, float y, const Color& color, const std::string& text) {
    ...
    golias::RID text_texture = load_texture_from_memory(rgba_surface->pixels, rgba_surface->w, rgba_surface->h, 4);

    if (text_texture == golias::INVALID_RID) {
    ...
    glm::vec4 tint_color  = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f); // White - color is already in texture
