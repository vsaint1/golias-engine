
#define ENGINE_NAME        "GOLIAS_ENGINE"
#define ENGINE_VERSION_STR "0.0.5"

#define ENGINE_DEFAULT_FOLDER_NAME "Golias Engine"
#define ENGINE_PACKAGE_NAME "com.golias.engine.app"

#define ALBEDO_TEXTURE_UNIT 0
#define NORMAL_MAP_TEXTURE_UNIT 1
#define METALLIC_ROUGHNESS_TEXTURE_UNIT 2
#define AMBIENT_OCCLUSION_TEXTURE_UNIT 3
#define EMISSIVE_TEXTURE_UNIT 4
#define SHADOW_TEXTURE_UNIT 5


/*!
* @defgroup Components
* @defgroup Systems
* @defgroup Tags
* @defgroup FileSystem
* @defgroup Configuration
* @defgroup Rendering
* @defgroup Time
* @defgroup Logging
*/
